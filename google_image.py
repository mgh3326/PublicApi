...
def load_url_files(_dir, file_name_prefix):
    ...
    if osp.exists(ttl_url_list_file_name):
        ...
        print
        'load URLs from file: ' + ttl_url_list_file_name
...
def get_real_url(url, loaded_urls):
    ...
    try:
        ...
        print
        'Real_url is: ' + str(real_url)

        if real_url in loaded_urls:
            print
            'URL had been downloaded in previous '
            real_url = None
...
for class_name, search_keywords in CONFIGS[u'search_keywords_dict'].iteritems():
    ...
    print
    'Loaded URLs in total is: ', loaded_urls_num

    # load pre-saved download parameters, actually cd_min for date range
    cd_max = cur_date

    params_file = osp.join(CONFIGS[u'save_dir'],
                           class_urls_file_prefix + '_params_' + CONFIGS[u'output_suffix'] + '.txt')
    print
    'Loaded pre-saved download parameters from: ' + params_file
    params_list = []
    ...
    class_save_dir = osp.join(CONFIGS[u'save_dir'],
                              class_urls_file_prefix + '_' + time_str + '_' + CONFIGS[u'output_suffix'])
    ...
    output_urls_file = osp.join(CONFIGS[u'save_dir'],
                                class_urls_file_prefix + '_' + time_str + '_' + CONFIGS[u'output_suffix'] + '.txt')
    ...
    while True:
        if cdr_enabled:
            ...
            print
            "==>Search for Images between " + cd_min.strftime("%Y/%m/%d") + \
            ' and ' + cd_max.strftime("%Y/%m/%d")
        else:
            cdr = ''
            print
            "==>Search for Images in any time"
        ...
        while j < len(search_keywords):
            print
            "\t==>Class name=" + str(class_name) + ', search keywords=' + search_keywords[j]
